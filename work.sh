...
"parsefile")
    if [ -z "$2" ] || [ -z "$3" ]; then
        echo "Usage: w parsefile <language> <file-number> [--force] [--verbose] [--debug]"
        echo "Example: w parsefile sl 001"
        echo "Example: w parsefile hr 042"
        echo "Example: w parsefile sl 123 --force --verbose"
        exit 1
    fi
    ...
    cmd="python parser/parsefile $2 $3"
    ...
    echo "🎵 Parsing single file using improved universal parser..."
    eval $cmd
    ;;

"chordprofile")
    echo "⚠️  Command 'chordprofile' is deprecated. Use 'parsefile' instead."
    echo "Example: w parsefile sl 001"
    echo "Example: w parsefile hr 042"
    exit 1
    ;;
...
"chordprofolder")
    echo "⚠️  Command 'chordprofolder' is deprecated. Use 'parsefolder' instead."
    echo "Example: w parsefolder sl"
...
"cleanup-logs")
    ...
    # Build command with all arguments
    cmd="python parser/cleanup-logs"
    ...
    echo "🧹 Managing parsing log files..."
    eval $cmd
    ;;
